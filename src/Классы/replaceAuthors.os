...
Процедура ПослеПолученияТаблицыВерсий(ТаблицаВерсий, ПутьКХранилищу) Экспорт
	...
	Для Каждого СтрокаВерсии Из ТаблицаВерсий Цикл
		...
		НужноЗаменитьАвтора = СтрНайти(СтрокаВерсии.Комментарий, МеткаЗамены) > 0;

		Если НужноЗаменитьАвтора Тогда
			
			РВ = Новый РегулярноеВыражение(МеткаЗамены + "\s+(\S+?)(\s|$)\s*$");
			Совпадения = РВ.НайтиСовпадения(СтрокаВерсии.Комментарий);
			Если Совпадения.Количество() Тогда
				ПервоеСовпадение = Совпадения[0];
				НовыйАвтор = ПервоеСовпадение.Группы[1].Значение;
				НовыйКомментарий = РВ.Заменить(СтрокаВерсии.Комментарий, "");
				СтарыйАвтор = СтрокаВерсии.Автор;

				СтрокаВерсии.Автор       = НовыйАвтор;
				СтрокаВерсии.ГУИД_Автора = НовыйАвтор;
				
				СтрокаВерсии.Комментарий = НовыйКомментарий;
			...
			Иначе
				...
				Лог.Предупреждение(
					"Не удалось переопределить Автора по метке комментария " + МеткаЗамены + "
					|Возможно после метки отсуствует Значение.
					|Комментарий:
